...
static int UG_vacuumer_get_block_data( struct UG_vacuumer* vacuumer, struct UG_vacuum_context* vctx, struct SG_manifest* block_requests ) {
   ...
   rc = UG_consistency_manifest_download( gateway, &reqdat, vctx->inode_data.coordinator, UG_RG_context_RG_ids( vctx->rg_context ), UG_RG_context_num_RGs( vctx->rg_context ), old_manifest, NULL );
